...
if (isset($_POST['rew'])) {
	mysqli_query($link, "INSERT INTO reviews VALUES(NULL, '{$_POST['author']}', '{$_POST['name']}', '{$_POST['review']}', CURRENT_DATE())");
}
?>
<!DOCTYPE html>
<html>
...
<body>
				...
				<div class="collapse navbar-collapse" style="margin-left: 1%; padding-right: 0px!important;" id="navbarNavDropdown">
					<ul class="navbar-nav">
						...
						<li class="nav-item" style="margin-left:7%;">
							<a class="nav-link text-white act hov" href="rew.php">Відгуки</a>
						</li>
						<li class="nav-item" style="margin-left:7%;">
	...
	<form method="POST">
		<div class="container">
			<input type="text" <?php echo "value='{$u[1]}'"; ?> name="author" class="form-control my-3" placeholder="Автор" aria-label="Username" aria-describedby="basic-addon1"/>
			<input type="text" name="name" class="form-control my-3" placeholder="Заголовок" aria-label="Username" aria-describedby="basic-addon1"/>
			<div class="form-floating">
				<textarea class="form-control" name='review' placeholder="Leave a comment here" id="floatingTextarea2" style="height: 200px"></textarea>
				<label for="floatingTextarea2">Відгук</label>
			</div>
			<button class="btn btn-danger my-3" name="rew">Додати відгук</button>
		</div>
	</form>
	...
	<div class="container my-5">
			...
			while ($row = mysqli_fetch_array($reviews)) {
				...
				echo "<h3 class='card-title my-3 text-center'>{$row[2]}</h3>";
				echo "<p class='card-text my-3 text-center'>{$row[3]}</p>";
				echo "</div>";
				echo "</div>";
			}
			?>
